...
(() => {
  ...
  function pintarDemo() {
    ...
    cont.innerHTML = '<h2>⏳ Versión de prueba</h2>' +
      '<div class="totalizador">' +
        '<div><span>Te queda</span><b>' + Demo.textoRestante() + '</b></div>' +
    ...
    $('#btnActivarDemo').onclick = async () => {
      const boton = $('#btnActivarDemo');
      boton.disabled = true;
      aviso('Comprobando…');
      if (await Demo.llaveCorrecta($('#llaveDemo').value)) {
        await Demo.liberar();
        pintarDemo();
        aviso('¡Listo! Este celular ya tiene la versión completa', 'exito');
      } else {
        aviso('Esa llave no es correcta', 'error');
